...
class TestViews(TestCase):
    ...
    def tearDown(self):
        models.Game.objects.include_inactive().exclude(id=1).delete()
    ...
    def test_program_level_creates_level(self):
        # Check DB not polluted before we start
        assert not models.Game.objects.filter(levelattempt__user=self.user, levelattempt__level_number=1).exists()
        c = self.login()
        response = c.get(reverse('aimmo/program_level', kwargs={'num': 1}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_mark_complete(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(models.Game.objects.include_inactive().get(id=1).completed)

    def test_mark_complete_for_non_existant_game(self):
        c = Client()
        response = c.post(reverse('aimmo/complete_game', kwargs={'id': 3})+'?auth_token=auth')
        self.assertEqual(response.status_code, 404)
        self.assertFalse(models.Game.objects.include_inactive().get(id=1).completed)
    ...
    def test_mark_complete_requires_POST(self):
        c = Client()
        response = c.get(reverse('aimmo/complete_game', kwargs={'id': 3})+'?auth_token=auth')
        self.assertNotEqual(response.status_code, 200)
        self.assertFalse(models.Game.objects.include_inactive().get(id=1).completed)
