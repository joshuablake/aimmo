...
class KubernetesWorkerManager(WorkerManager):
    ...
    def _create_game_rc(self, id, environment_variables):
        ...
        environment_variables['GAME_API_URL'] =\
            'https://staging-dot-decent-digit-629.appspot.com/aimmo/api/games/%s' % id
                            ...
                            'paths': [
                                {
                                    'path': "/game/%s/*" % id,
                                    'backend': {
                                        'serviceName': "game-%s" % id,
                                        'servicePort': 80,
                                    },
                                } for id in self._data.get_games()
                            ],
                        },
                    }],
                },
