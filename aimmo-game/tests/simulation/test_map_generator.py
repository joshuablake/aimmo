...
from .dummy_avatar import EmptyAvatarManager
...
class _BaseGeneratorTestCase(unittest.TestCase):
    def get_game_state(self, **kwargs):
        ...
        return self.GENERATOR_CLASS(settings).get_game_state(EmptyAvatarManager())
...
class TestLevel1Generator(_BaseGeneratorTestCase):
    ...
    def test_completes_at_score_1(self):
        game_state = self.get_game_state()
        game_state.avatar_manager.add_avatar(1, '', None)
        game_state.avatar_manager.avatarsById[1].score = 1
        game_state.main_avatar_id = 1
    ...
    def test_static_spawn(self):
        ...
        for i in xrange(5):
            ...
            self.assertEqual(game_state.avatar_manager.avatarsById[i].location, Location(0, 0))
